...
import * as cartUtils from "../../refactoring/features/cart/utils/calculation";
import { Product } from "../../refactoring/features/product/types";
import { Coupon } from "../../refactoring/features/coupon/types";
import { CartItem } from "../../refactoring/features/cart/types";
  ...
  describe("cartUtils", () => {
    ...
    describe("calculateItemDiscount", () => {
      test("할인이 적용되지 않으면 0을 반환해야 합니다.", () => {
        const item: CartItem = { product: testProduct, quantity: 1 };
        expect(cartUtils.calculateItemDiscount(item)).toBe(0);
      });

      test("적용 가능한 가장 높은 할인율을 반환해야 합니다.", () => {
        const item: CartItem = { product: testProduct, quantity: 5 };
        expect(cartUtils.calculateItemDiscount(item)).toBe(100);
      });
    });
    ...
    describe("calculateCartTotal", () => {
      ...
      test("쿠폰 없이 총액을 올바르게 계산해야 합니다.", () => {
        const result = cartUtils.calculateCartTotals(cart, null);
      ...
      test("금액쿠폰을 올바르게 적용해야 합니다.", () => {
        ...
        const result = cartUtils.calculateCartTotals(cart, coupon);
